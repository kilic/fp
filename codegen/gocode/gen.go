...
func main() {
	...
	if *genFields {
		...
		GenerateFields(*out+"/"+"fields.go", *from, *to)
		GenerateFieldElementTests(*out+"/"+"field_test.go", *from, *to)
	}
	if *genDeclarations {
		GenerateDeclerations(*out+"/"+"arithmetic_decl.go", *from, *to)
...
func GenerateFields(out string, from, to int) {
	pkg := "fp"
...
func GenerateDeclerations(out string, from, to int) {
	s := "package fp\n\n"
